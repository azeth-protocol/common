 ...
 */
function roundFraction(fractionStr: string, digits: number): { value: string; carry: boolean } {
  if (digits >= fractionStr.length) return { value: fractionStr, carry: false };
  if (digits === 0) {
  ...
  if (nextDigit < 5) return { value: kept, carry: false };
...
export function formatTokenAmount(amount: bigint, decimals: number, maxFractionDigits?: number): string {
  ...
  if (maxFractionDigits !== undefined) {
    ...
    if (carry) whole += 1n;
    if (rounded === '' || /^0+$/.test(rounded)) return `${sign}${whole.toString()}`;
